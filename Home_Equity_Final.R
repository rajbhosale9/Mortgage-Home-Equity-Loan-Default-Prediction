...
plot(Home_equity_new$Default, main = "Default", col = "steel blue", 
...
sil_plot(dist_mat = dist,
         ...
         max.k = 15)
## 11
## Possible values are 2, 10 and 11
...
aggregate(x = Home_equity_new[ ,numerical], 
...
aggregate(x = Home_equity_new[ ,c(nominal)], 
          by = list(wards_clusters), 
          FUN = table)
...
## View the Medoids.
Home_equity_new[pam$medoids,]

# We can use the matplot() function to
# visualize the (scaled) cluster centers
# to observe differences across clusters
matplot(t(home_equity_yj[pam$medoids, numerical]), # transposed, scaled medoids
        type = "l", # line plot ("l")
        ylab = "", # no y-axis label
        xlim = c(0, 8), # add space for legend
        xaxt = "n", # no x-axis
        col = 1:8, # 8 colors, k = 8
        lty = 1:8, # 8 line types, k = 8
        main = "Cluster Centers") # main title
# Add custom x-axis labels
axis(side = 1, # x-axis
     at = 1:7, # x values 1-7 (7 numeric variables)
...
par(mfrow = c(1,1)) # revert plot window to 1 row, 1 column
...
nb_train_conf_ROS <- confusionMatrix(data = nb_train_ROS, # predictions
                                  reference = nb_train_up$Default, # actual
                                  positive = "1",
                                  mode = "everything")
nb_train_conf_ROS
...
nb_test_ROS <- predict(object = nb_fit_ROS, # NB model
                       newdata = test[ ,vars], # predictors
                       type = "class")

nb_test_conf_ROS <- confusionMatrix(data = nb_test_ROS, # test predictions
